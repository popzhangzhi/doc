...
## 2 服务提供者 service provider：
		register 只用与绑定服务容器(service container)
		boot 用于事件监听 composer绑定视图，并可以访问可加重已注册的所有服务（该方法会在所有访问提供者注册完成后才被调用）
## 3 注册服务提供者：
...
### 4.8 关于服务提供者、服务容器、facedes三者的总结
	...
	服务容器（$this->app）各种形式绑定*命名空间*到对应的*类*，单例、实例、接口、上下文绑定、原始值、标签。以一种容器的概念去调整运行中实际运行的类，从而实现面相接口的编程。主要用类似工厂模型的方式，进行整个框架的解耦，实现依赖反转。
	
	facedes是服务容器中 '静态'的提供接口，方便快捷,静态调用，需要服务绑定。
